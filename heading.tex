...
\title{An Open Source Conceptual Sizing Model for the Hyperloop Passenger Pod}

%%% first author
\author{Jeffrey C. Chin
    \affiliation{
	Research Aerospace Engineer\\
	NASA Glenn Research Center\\
	Propulsion Systems Analysis Branch\\
	Cleveland OH, 44135\\
    Email: jeffrey.c.chin@nasa.gov
    }	
}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{abstract}
{\it Hyperloop is a transportation concept proposed as a faster,
...
\begin{nomenclature}
\entry{A}{Area}
\entry{BF}{Blockage Factor, diffused area over pod area}
\entry{${C_{p}}$}{Heat capacity at constant pressure}
\entry{${c_{solar}}$}{Gross Irradiance Adjustment}
\entry{D}{Diameter}
\entry{g}{Acceleration due to gravity}
\entry{G}{Solar Irradiance}
\entry{Gr}{Grashof Number}
\entry{h}{Heat transfer coefficient}
\entry{k}{Thermal Conductivity}
\entry{L}{Length}
\entry{MN}{Mach Number}
\entry{$\dot{m}$}{Mass flow rate}
\entry{n}{Number of Pods}
\entry{NU}{Nusselt Number}
\entry{P}{Pressure}
\entry{Pr}{Prandtl Number}
\entry{Q}{Heat flow rate}
\entry{r}{Radius}
\entry{Ra}{Rayleigh Number}
\entry{T}{Temperature}
\entry{$\alpha$}{Thermal Diffusivity}
\entry{$\beta$}{Volume coefficient of expansion}
\entry{$\gamma$}{Heat Capacity Ratio}
\entry{$\epsilon$}{Total, Hemispherical Emissivty}
\entry{${\eta}_{c}$}{Adiabatic Compressor Efficiency}
\entry{${\pi}_{c}$}{Compressor Pressure Ratio}
\entry{${\rho}_{r}$}{Total, Hemispherical Reflectivity}
\entry{$\sigma$}{Stefan-Boltzmann constant}
\entry{$\upsilon$}{Kinematic Viscosity}
...
\begin{figure}[hbtp]
\centering
\includegraphics[width=3.34in]{images/TopAssembly.png}
...
\begin{figure}[hbtp]
\centering
\includegraphics[width=3.34in]{images/podAssembly.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=3.34in]{images/compAssembly.png}
...
\begin{figure}[hbtp]
\centering
\includegraphics[width=3.34in]{images/closedCapsule2.png}
...
\begin{figure}[!htb]
  \centering
  \includegraphics[width=3.34in]{images/areaPlot.pdf}
...
\begin{figure}[hbt]
\centering
\includegraphics[width=3.34in]{images/hollowCapsule2.png}
...
\begin{figure}[hbtp]
\centering
\includegraphics[width=3.34in]{images/openCapsule2.png}
...
\begin{figure}[hbt]
\centering
\includegraphics[width=3.34in]{images/mach_vs_rad6.png}
...
\begin{figure}[hbtp]
\centering
\includegraphics[width=3.34in]{images/velocity_profile3.png}
...
\begin{figure}[hbtp]
\centering
\includegraphics[width=3.34in]{images/battery_plot.pdf}
...
\begin{equation}
\underbrace{\dot{m}_{air} C_{p, air} (T_{out, air} - T_{in, air})}_{{Q}_{released}}  = \underbrace{\dot{m}_{H_{2}O} C_{p,H_{2}O} (T_{out, H_{2}O} - T_{in, H_{2}O})}_{{Q}_{absorbed}}
\end{equation}
...
\begin{table}
    ...
    \begin{tabular}{l c c} 
        \hline
        Property & Air & Water \\ \hline
        $C_{p}$ ($\frac{kJ}{kg-K}$)&  1.006 & 4.186 \\
        $T_{in}$ (K) & 791 & 288.15 \\
        $T_{out}$ (K) & 300 & 416.6 \\
        $\dot{m}$ ($\frac{kg}{s}$) & 0.49 &  0.45 \\
        Q ($\frac{kJ}{s}$) & -242 & 242 \\
        $Q_{max}$ & 247.9 & 247.9\\ \hline 
    \end{tabular}
\end{table}
...
\begin{table}[hbtp]
    ...
    \begin{tabulary}{0.8\textwidth}{l  c  c  c c c} 
        \hline
         & Spread & Min & Mode & Max & Std. Dev\\ \hline \hline
        $T_{amb}$ &  Normal & - & 305 K & - & 4.5 K \\ 
        $G$ & Triangle & $200\frac{W}{m^{2}}$ &  $1000\frac{W}{m^{2}}$ & $1000\frac{W}{m^{2}}$ & - \\
        $\rho$ & Triangle & 0.4 & 0.5 &  0.9 & - \\
        $\epsilon$ & Triangle & 0.4 & 0.5 & 0.9 & - \\
        $\eta_{c}$ & Triangle & 0.6 & 0.69 & 0.8 & - \\
        $n$ & Triangle & - & 34 & - & 2 \\
        $c_{solar}$ & Triangle & 0.5 & 0.7 & 1.0 & - \\
        $c_{conv}$ & Triangle & 0.9 & 1.0 & 3.0 & - \\
        \hline
    \end{tabulary}
...
\begin{figure}[hbtp]
\centering
\includegraphics[width=3.34in]{images/histo.pdf}
...
\section*{Appendix A: Source Code} \label{app:Github}

The presented models were built exclusively using freely-available, open-source tools.
The code requires OpenMDAO, a framework serving as the backbone for subsystem integration,
optimization and workflow management.
As the Hyperloop design evolves,
an ever increasing number of design variables will need to be managed and optimized.
...
This plugin is designed to be a jumping off point for community contributions
to help crowd source the development of the Hyperloop
concept. The readme on the github repository walks through basic installation steps,
further support can be found through the main OpenMDAO docs.
...
\begin{figure}[hbtp]
\centering
\includegraphics[width=3.34in]{images/hyperloop_cad.png}
